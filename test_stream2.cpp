...
vector<OutputBlock> process_buffer(
    const vector<vector<vector<char>>> &buffer,
    const BlockInfo &block_info,
    const map<char, string> &tag_table
) {
    vector<OutputBlock> compressedOutput = compress(block_info, tag_table, buffer);
...
int main()
{
    ...
    try {
        ...
        buffer.resize(block.z_count, vector<vector<char>>(block.y_count, vector<char>(block.x_count)));
        for(z_read = 0; z_read < block.parent_z; ++z_read) {
            // Read one slice into blockModel (using z_read as the slice index)
            read_blockModel(blockModel, block);

            // Copy the slice from blockModel to buffer
            buffer[z_read] = blockModel[z_read];

            // Perform something when z_read % block.parent_z == 0
            if (z_read % block.parent_z == 0 && z_read != 0) {
                BlockInfo sub_block;
                sub_block.z_count = blockModel.size();
                
                vector<OutputBlock> compressedOutput = compress(block, tagTable, buffer);
