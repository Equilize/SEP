...
// Globals
queue<vector<vector<vector<char>>>> readQueue;
queue<vector<OutputBlock>> compressQueue;
mutex readMutex, compressMutex;
condition_variable readCond, compressCond;
BlockInfo globalBlockInfo;
map<char, string> globalTagTable;
bool readDone = false; // To indicate that reading is done
...
void read_input(BlockInfo &block, map<char, string> &tagTable, vector<vector<vector<char>>> &blockModel) {
    ...
    //           << block.parent_x << ", " << block.parent_y << ", " << block.parent_z << endl;
...
void read_input_initial(BlockInfo &block, map<char, string> &tagTable) {
    auto splitString = [](const string &s, char delimiter) {
        vector<string> tokens;
        string token;
        istringstream tokenStream(s);
...
void read_blockmodel_stage() {
    while (true) {
        vector<vector<vector<char>>> blockModel;
        blockModel.resize(globalBlockInfo.z_count, vector<vector<char>>(globalBlockInfo.y_count, vector<char>(globalBlockInfo.x_count)));
        string line;
        // Reading block model content
        for(int z = 0; z < globalBlockInfo.z_count; ++z) {
            for(int y = 0; y < globalBlockInfo.y_count; ++y) {
                ...
                if (line.length() != static_cast<size_t>(globalBlockInfo.x_count)) {
                    stringstream ss;
                    ss << "Block content width and x_count does not match. ";
                    ss << "The error occurred at slice (z): " << z << ", row (y): " << y;
                    throw runtime_error(ss.str());
            ...
            if (z != globalBlockInfo.z_count - 1) {  // No need to read blank line after the last slice
                getline(cin, line);
            } else {
                // Check if this is the last slice of the chunk
                // If so, check for a blank line or EOF
                if (!cin.eof()) {
                    getline(cin, line);
                    if (!line.empty()) {
                        // Handle the case where you expected a blank line but didn't get one
                        // For example, you could throw an error or set a flag
                        throw runtime_error("Expected a blank line after the last slice of the chunk.");
                    }
                }
            }
        }
        ...
        if (cin.eof() || cin.fail()) {
            ...
            readCond.notify_all(); // Notify all waiting threads
            break;
        }

        {
            unique_lock<mutex> lock(readMutex);
            readQueue.push(blockModel);
        }
        readCond.notify_one(); // Notify compress_stage
    }
}
void compress_stage() {
    while (true) {
        ...
        {
            unique_lock<mutex> lock(readMutex);
            readCond.wait(lock, [] { return !readQueue.empty() || readDone; });
            if (readQueue.empty() && readDone) break;
            blockModel = readQueue.front();
            readQueue.pop();
        }

        vector<OutputBlock> compressedOutput = compress(globalBlockInfo, globalTagTable, blockModel);
        {
            unique_lock<mutex> lock(compressMutex);
            compressQueue.push(compressedOutput);
        }
        compressCond.notify_one(); // Notify write_stage
    }
}
...
void write_stage() {
    while (true) {
        ...
        {
            unique_lock<mutex> lock(compressMutex);
            compressCond.wait(lock, [] { return !compressQueue.empty() || readDone; });
            if (compressQueue.empty() && readDone) break;
            compressedOutput = compressQueue.front();
            compressQueue.pop();
        }
...
int main() {
    // Read BlockInfo and tagTable only once
    read_input_initial(globalBlockInfo, globalTagTable);

    // Start pipeline
    thread read_thread(read_blockmodel_stage);
    thread compress_thread(compress_stage);
    thread write_thread(write_stage);
