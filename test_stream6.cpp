...
bool is_homogeneous(const vector<vector<vector<char>>> &bufferModel, int x, int y, int z, int x_size, int y_size, int z_size, char tag) {
    cout << "Starting is_homogeneous..." << endl;
    for(int dz = 0; dz < z_size; dz++) {
        for(int dy = 0; dy < y_size; dy++) {
            for(int dx = 0; dx < x_size; dx++) {
                if (bufferModel[z + dz][y + dy][x + dx] != tag) {
                    return false;
                }
            }
        }
...
vector<OutputBlock> compress_block(int x, int y, int z, int x_size, int y_size, int z_size, const BlockInfo &block, const map<char, string> &tagTable, const vector<vector<vector<char>>> &bufferModel) {
    cout << "Starting compress_block ..." << endl;
    //cout << "In compress_block, processing block of size: (" << x_size << ", " << y_size << ", " << z_size << ")" << endl;

    vector<OutputBlock> blocks;
    cout << "Maybe its here0" << endl;
    char tag = bufferModel[z][y][x];
    cout << "Maybe its here1" << endl;
    auto it = tagTable.find(tag);
    cout << "Maybe its here2" << endl;
    if (it == tagTable.end()) {
        throw runtime_error("Unknown tag encountered.");
    }
    
    if (is_homogeneous(bufferModel, x, y, z, x_size, y_size, z_size, tag)) {
        ...
        for (int dx = 0; dx < x_size; dx += half_x) {
            for (int dy = 0; dy < y_size; dy += half_y) {
                for (int dz = 0; dz < z_size; dz += half_z) {
                    auto innerBlocks = compress_block(x+dx, y+dy, z+dz, min(half_x, x_size-dx), min(half_y, y_size-dy), min(half_z, z_size-dz), block, tagTable, bufferModel);
...
vector<OutputBlock> compress(const BlockInfo &block, const map<char, string> &tagTable, const vector<vector<vector<char>>> &blockContent) {
    ...
    //cout << "In compress, bufferModel has z-size: " << blockContent.size() << endl;
