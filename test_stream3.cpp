...
vector<OutputBlock> process_buffer(
    ...
    const BlockInfo &sub_block,
    const map<char, string> &tagTable
) {
    vector<OutputBlock> compressedOutput = compress(sub_block, tagTable, buffer);
...
int main()
{
    ...
    try {
        ...
        while (z_read < block.z_count) {
            ...
            for (int y = 0; y < block.y_count; ++y) {
                for (int x = 0; x < block.x_count; ++x) {
                    buffer[z_read][y][x] = blockModel[z_read][y][x];
                }
            }
            ...
            if (z_read % block.parent_z == 0 ) {
                ...
                vector<OutputBlock> compressedOutput = process_buffer(buffer, sub_block, tagTable);
