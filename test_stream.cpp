...
condition_variable readDoneCond; // Condition variable for readDone
...
void read_stage() {
    ...
    try {
        read_input(globalBlockInfo, globalTagTable, blockModel);
        {
            unique_lock<mutex> lock(readMutex);
            readQueue.push(blockModel);
            readDone = true; // Signal that reading is done
        }
        readDoneCond.notify_all(); // Notify all waiting threads
        readCond.notify_one(); // Notify compress_stage
    }
    catch(const runtime_error &e) {
        cerr << "Error in read_stage: " << e.what() << endl;
    }
}
void compress_stage() {
    vector<vector<vector<char>>> blockModel;
    try {
        {
            unique_lock<mutex> lock(readMutex);
            readDoneCond.wait(lock, [] { return readDone; }); // Wait until reading is done
            readCond.wait(lock, [] { return !readQueue.empty(); }); // Wait until readQueue has data
        ...
        cerr << "Error in compress_stage: " << e.what() << endl;
    }
}
...
void write_stage() {
    ...
    try {
        {
            unique_lock<mutex> lock(compressMutex);
            compressCond.wait(lock, [] { return !compressQueue.empty(); });
        ...
        cerr << "Error in write_stage: " << e.what() << endl;
    }
}

int main() {
    thread read_thread(read_stage);
