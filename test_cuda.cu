...
__global__ void is_homogeneous_kernel(const char *blockModel, int x, int y, int z, int x_size, int y_size, int z_size, int x_count, int y_count, int z_count, char tag, bool *result) {
    // Note: This kernel is not optimized for performance, but rather for simplicity.
    int dx = threadIdx.x;
    int dy = threadIdx.y;
    int dz = threadIdx.z;

    if (dx < x_size && dy < y_size && dz < z_size) {
        int index = (z + dz) * (y_count * x_count) + (y + dy) * x_count + (x + dx);
        if (blockModel[index] != tag) {
            *result = false;
        }
    }
}
...
bool is_homogeneous(const vector<vector<vector<char>>> &blockModel, int x, int y, int z, int x_size, int y_size, int z_size, char tag) {
    // Flatten blockModel for easy GPU memory copying
    int x_count = blockModel[0][0].size();
    int y_count = blockModel[0].size();
    int z_count = blockModel.size();
    char *flatBlockModel = new char[x_count * y_count * z_count];

    for (int dz = 0; dz < z_count; ++dz) {
        for (int dy = 0; dy < y_count; ++dy) {
            for (int dx = 0; dx < x_count; ++dx) {
                flatBlockModel[dz * (y_count * x_count) + dy * x_count + dx] = blockModel[dz][dy][dx];
            }
        }
    }
    ...
    bool h_result = true;
    bool *d_result;
    cudaMalloc(&d_blockModel, x_count * y_count * z_count * sizeof(char));
    cudaMalloc(&d_result, sizeof(bool));
    cudaMemcpy(d_blockModel, flatBlockModel, x_count * y_count * z_count * sizeof(char), cudaMemcpyHostToDevice);
    cudaMemcpy(d_result, &h_result, sizeof(bool), cudaMemcpyHostToDevice);

    // Launch kernel
    dim3 threadsPerBlock(x_size, y_size, z_size);
    is_homogeneous_kernel<<<1, threadsPerBlock>>>(d_blockModel, x, y, z, x_size, y_size, z_size, x_count, y_count, z_count, tag, d_result);

    // Copy the result back
    cudaMemcpy(&h_result, d_result, sizeof(bool), cudaMemcpyDeviceToHost);

    // Free device memory
    cudaFree(d_blockModel);
    cudaFree(d_result);
    delete[] flatBlockModel;
