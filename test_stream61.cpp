// Version 1.0.3 (Stream Support) Change blockModel to blockmodel
#include <iostream>
#include <vector>
#include <string>
#include <map>
...
vector<OutputBlock> compress(const BlockInfo &block, const map<char, string> &tagTable, const vector<vector<vector<char>>> &blockContent) {
    ...
    //cout << "In compress, blockModel has z-size: " << blockContent.size() << endl;
...
void processBuffer(const BlockInfo &block, const map<char, string> &tagTable) {
    ...
    blockModel.clear();  // Clear the buffer for the next chunk
}

void read_stream(BlockInfo &block, map<char, string> &tagTable) {
    blockModel.resize(BUFFER_SIZE);
    string line;
    int z = 0;
    ...
    while (true) {  
        ...
        for (; z < BUFFER_SIZE && global_z < block.z_count; ++z, ++global_z) {
            //cout << "Filling blockModel at z-index: " << z << endl;  // Debug line

            blockModel[z].resize(block.y_count, vector<char>(block.x_count));
            for (int y = 0; y < block.y_count; ++y) {
                getline(cin, line);
                if (line.empty()) {
                    getline(cin, line);
        ...
        if (z > 0) {
            // Debug line to print blockModel's content
            // cout << "Current blockModel content:" << endl;
            // for (int z = 0; z < blockModel.size(); ++z) {
            //     cout << "Slice " << z << ": " << endl;
            //     for (int y = 0; y < block.y_count; ++y) {
            //         for (int x = 0; x < block.x_count; ++x) {
            //             cout << blockModel[z][y][x];
