...
def print_final_stats_and_exit(expanded_count, compressed_count):
    cleanup_subprocesses()
    close_files()
    ...
    if ARGS.verbose:
        printerr_item_under_test_stderr()
    printlog("-----------")
    compression = (expanded_count - compressed_count) / expanded_count
    printlog("Compression = {:6.2f}%  ({} blocks down from {} blocks)".format(
        compression*100,
        compressed_count,
        expanded_count))
    if ARGS.speed:
        speed = expanded_count/ARGS.runtime
    ...
    while True:
        ...
        if zT >= bz_end:
            ...
            if line_countT % 1024 == 0 and timeit.default_timer() - overall_tic > ARGS.runtime:
                print_final_stats_and_exit(block_countT, line_countT)
